...
use App\Http\Requests\Booking\MasterUpdateRequest;
...
class BookingController extends MasterBaseController
{
    use Notification;
     ...
     *
     * @param MasterStoreRequest $request
     * @return JsonResponse
     */
    public function store(MasterStoreRequest $request): JsonResponse
    {
     ...
     * @return JsonResponse
     */
    public function calculate(MasterStoreRequest $request): JsonResponse
    {
        $validated = $request->validated();
    ...
    {
        if ($booking->master_id !== auth('sanctum')->id()) {
     ...
     * @return JsonResponse
     */
    public function update(Booking $booking, MasterUpdateRequest $request): JsonResponse
    {
        if ($booking->master_id !== auth('sanctum')->id()) {
            return $this->onErrorResponse([
                'code'    => ResponseError::ERROR_404,
    ...
    {
        $bookings = $this->repository->bookingsByParentId($id, masterId: auth('sanctum')->id());
    ...
    public function destroy(FilterParamsRequest $request): JsonResponse
    {
        $this->service->delete(
            $request->input('ids', []),
            $request->merge(['master_id' => auth('sanctum')->id()])->all()
        );
